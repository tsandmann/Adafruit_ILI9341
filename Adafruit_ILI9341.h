...
public:
    /*!
     *  @brief  Instantiate Adafruit ILI9341 driver with hardware SPI using the default SPI peripheral.
     ...
     *  @param  rst       Reset pin # (optional, pass -1 if unused).
     */
    Adafruit_ILI9341(SPIClass* spiClass, int8_t cs, int8_t dc, int8_t rst = -1);
    ...
    /*!
     *  @brief  Set the "address window" - the rectangle we will write to RAM with the next chunk of SPI data writes.
     *          The ILI9341 will automatically wrap the data as each row is filled.
     *  @param  x1  TFT memory 'x' origin
     *  @param  y1  TFT memory 'y' origin
     *  @param  w   Width of rectangle
     *  @param  h   Height of rectangle
     */
    virtual void setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) const override;
     ...
     *  @note    This is highly undocumented/supported, it's really a hack but kinda works?
     */
    uint8_t readcommand8(uint8_t reg, uint8_t index = 0);

private:
    static constexpr uint32_t SPI_DEFAULT_FREQ { 24'000'000 }; /*!< Default SPI frequency */
};
